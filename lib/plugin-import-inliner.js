 ...
 */
export const inlineImportsFromGithub = async (entryPoint, codeBlockString, functionTranslations) => {
  ...
  for (const importUrl of importUrls) {
    ...
    if (functionBlocks) {
      for (let [ functionName, functionBlock ] of Object.entries(functionBlocks)) {
        ...
        const newFunctionName = `_inlined_${ urlSegments[urlSegments.length - 1] }_${ functionName }`;
...
const fileContentFromUrl = async (url) => {
  let fileContent;
  console.log("Checking presence of", url, "Do you read me Houston?");
