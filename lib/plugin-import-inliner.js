 ...
 */
export async function inlineImports(entryPoint, codeBlockString, functionTranslations, source) {
  let importUrlsFunction, getFileContentFunction;
  if (source === "github") {
    importUrlsFunction = importUrlsFromGithub;
    getFileContentFunction = fileContentFromUrl;
  }
  if (source === "offline") {
    importUrlsFunction = importUrlsOffline;
    getFileContentFunction = fileContentOffline;
  }
  return await _inlineImports(
    entryPoint, codeBlockString, functionTranslations, importUrlsFunction, getFileContentFunction
  );
}

export async function _inlineImports(entryPoint, codeBlockString, functionTranslations, importUrlsFunction, getFileContentFunction) {
  const { content, url } = entryPoint;
  if (!content) return null;

  const importUrls = importUrlsFunction.bind(this)(content, url);
  ...
  for (const importUrl of importUrls) {
    ...
    const importFileContent = await getFileContentFunction.bind(this)(importUrl);
    ...
    if (functionBlocks) {
      for (let [ functionName, functionBlock ] of Object.entries(functionBlocks)) {
        ...
        const isGenerator = /function\*/.test(definition);
    ...
    codeBlockString = await _inlineImports(
      { url: importUrl, content: importFileContent },
      codeBlockString, functionTranslations, importUrlsFunction, getFileContentFunction
    );
  }
...
function fileContentOffline(filePath) {
  // Assuming UTF-8 encoding. Modify if different encoding is needed.
  try {
    return fs.readFileSync(filePath, 'utf8');
  } catch (err) {
    console.error(`Unable to read file ${filePath}`, err);
    return null;
  }
}

//----------------------------------------------------------------------
const importUrlsFromGithub = (content, contentFileUrl) => {
  const extension = contentFileUrl.split(".").pop();
  let match;
  const importUrls = [];
...
const importUrlsOffline  = (content, contentFilePath) => {
  const extension = path.extname(contentFilePath).substring(1); // Get file extension without dot
  let match;
  const importPaths = [];
  const importRegex = /import\s+\{\s*([^}]+)\s*}\s+from\s+['"]([^'"]+)['"]/g;
  ...
  while ((match = importRegex.exec(content)) !== null) {
    ...
    if (importPath.startsWith("./")) {
      importPath = path.resolve(path.dirname(contentFilePath), importPath);
    } else {
      // Assume that non-relative paths are module imports and not local files
      continue;
    }
    if (!/\.[jt]s$/.test(importPath)) {
