    ...
    if (functionBlocks) {
      for (let [ functionName, functionBlock ] of Object.entries(functionBlocks)) {
        ...
        functionTranslations.forEach(translation => {
          functionBlock = functionBlock.replaceAll(`${ translation.functionName }(`, `this.${ translation.newFunctionName }(`);
        })
...
async function functionBlocksFromFileContent(fileContent) {
  let result = {};
  const functionRegex = /^(?:export\s+)?((?:async\s+)?function\s+(?<functionName>[^\s\(]+)\s*\(|(?:const|let)\s+(?<variableName>[^\s=])\s*=)/gm;
