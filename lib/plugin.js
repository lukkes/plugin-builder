...
const plugin = {
  ...
  */
  async _codeBlockFromGithub(app, entryPoint) {
    ...
    const moduleNames = importStatements.map(statement => {
      const parts = statement.split('from');
      if (parts.length !== 2) {
        return null;
      }
      const moduleName = parts[1].trim().replace(/['"]/g, '');
      return moduleName;
    }).filter(Boolean);

    const lines = content.split("\n");
    const importLines = lines.filter(line => line.startsWith("import"));
    const importUrls = importLines.map(line => line.replace(/^import\s+["']([^"']+)["'];?$/i, "$1"));
    const importPromises = importUrls.map(importUrl => this._entryPointFromUrl(app, importUrl));
    const importResults = await Promise.all(importPromises);
    const importBlocks = await Promise.all(importResults.map(importResult => this._codeBlockFromGithub(app, importResult)));
    const importBlock = importBlocks.join("\n\n");
    return `${ importBlock }\n\n${ content }`;
  },
