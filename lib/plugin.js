...
const plugin = {
  ...
  async _syncUrlToNote(app, repoUrl) {
    ...
    if (entryPoint.url) {
      ...
      console.log("Starting note content", noteContent)
      ...
      if (newPluginBlock) {
        if (newPluginBlock.length > this._constants.maxReplaceContentLength) {
          ...
          let newContent = noteContent.substring(0, noteInsertExtents[0]);
          newContent += `\`\`\`\n// Javascript inserted by Amplenote Plugin Builder () from source code at ${ repoUrl }\n${ newPluginBlock }\n\`\`\``;
          newContent += noteContent.substring(noteInsertExtents[1]);
          newContent = newContent.replace("{Github Plugin Builder: Plugin sync}", "");
