...
const plugin = {
  ...
  async _syncUrlToNote(app, repoUrl) {
    ...
    if (entryPoint.url) {
      ...
      const newCodeBlock = await this._codeBlockFromGithub(entryPoint)
      if (newCodeBlock.length > this._constants.maxReplaceContentLength) {
        ...
        noteContent = noteContent.substring(0, noteInsertExtents[0]) + newCodeBlock + noteContent.substring(noteInsertExtents[1]);
