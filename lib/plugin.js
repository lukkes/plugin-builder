...
const plugin = {
  ...
  async _syncUrlToNote(app, repoUrl) {
    ...
    if (entryPoint.url) {
      ...
      let newPluginBlock = await inlineImportsFromGithub(entryPoint, mainPluginBlock);
      if (newPluginBlock) {
        if (newPluginBlock.length > this._constants.maxReplaceContentLength) {
          ...
          newPluginBlock = `\`\`\`\n// Javascript inserted by Amplenote Plugin Builder (https://www.amplenote.com/plugins/FZf22PXCKTRTB1tJwta1Nepq) from source code at "${ repoUrl }"\n${ newPluginBlock }\n\`\`\``;
  ...
  async _isAbleToSync(app, noteContent) {
    if (noteContent.includes(this._constants.codeHeading)) {
      ...
      if (/^```/m.test(noteContent)) {
        await app.alert(`Could not sync plugin because the note already contains code but no code block heading. Please add ` +
   ...
   * @returns {array|null} - [ startMatchIndex, endMatchIndex ], where indexes are chosen such that they exclude the ``` characters
   */
  async _noteInsertRange(body) {
    ...
    const startMatchIndex = matches[0] ? matches[0].index : null;
    const endMatchIndex = matches[1] ? matches[1].index + matches[1][0].length : null;
