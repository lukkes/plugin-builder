import { inlineImportsFromGithub, fetchWithRetry } from "./plugin-import-inliner"
...
const plugin = {
  ...
  insertText: {
    ...
    "Sync": {
      check: async function(app) {
        return await this.insertText["Refresh"].check(app);
      },
      run: async function(app) {
        return await this.insertText["Refresh"].run(app);
      }
    }
  },
