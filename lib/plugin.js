import { objectBlocksFromImportMatch, pluginFetch } from "./plugin-file-extractor"
  ...
  */
  async _codeBlockFromGithub(app, entryPoint) {
    ...
    while ((match = importRegex.exec(content)) !== null) {
      const functions = match[1].split(",").map(part => part.trim());
      ...
      importStatements.push({ functions, importUrl });
    }

    let insertBlocks = [];
    for (const importMatch of importStatements) {
      // Returns { [functionName]: [functionCode minus leading "export"], ... }
      const objectBlocks = await objectBlocksFromImportMatch(importMatch);
      if (objectBlocks) {
        insertBlocks = { ...insertBlocks, ...objectBlocks };
      }
    }
